...
const fmt=v=>v==null||v===""?"":typeof v==="boolean"?(v?"Yes":"No"):Array.isArray(v)?v.map(x=>typeof x==="object"?JSON.stringify(x):String(x)).join(", "):typeof v==="object"?JSON.stringify(v):String(v);
...
const sections=[
 ...
 {heading:"Sign-Off",rows:pick([["technician_name","Technician"],["customer_rep","Customer Representative"],["customer_signature","Customer Signature"],["signature_date","Signed"]])},
];
const pdf=await buildSimplePdf({title:"Hood Cleaning Performance Report",subtitle:`${d.business_name}  •  ${d.service_date}`,sections,footer:"Inspection Clean  •  service@inspectionclean.com"});
